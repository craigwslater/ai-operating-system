...
## [2.0.2]—2026-08-21—Quantitative claim refresh; byte counts retired

A PATCH release—no new content. Quantitative claims re-derived from `~/aios/` source and corrected across the narrative and artifact tiers, plus one structural change to how size claims are published. Produced by Activation 4a of the maintenance project.

Worth stating plainly, because it is the more useful fact: the two walkers (`staleness-check.py`, `claim-verify.py`) surfaced ten of these corrections and the claim register moved from 11 DRIFT rows to 1—but **six of the corrections below were found by independent verification rather than by the walkers, and would not have been caught by re-running them.** The eval gloss, the "six release projects" qualifier, the detector count, Case Study #2's detector paragraph, the metrics baseline date and the artifact enumeration were all invisible to a tool that compares a source-derived value against a register row and never reads the pages the claim is published on. The detector count is the sharpest case: its row reported PASS throughout, because the row's own verify command counted `def` lines—the method the source had already rejected. That is the same coverage gap the register-pointer thread below describes, seen from the other side.

### Updated (2026-08-21)

- **Eval count 40 → 46**, the portfolio's most-repeated number: 17 occurrences across 11 files—10 narrative files plus the artifact tier (`artifacts/sample-skill/SKILL.md`). The claim register listed only 11 pointers across 7 files against those 17 occurrences: `methodology.md`, `retrospection.md`, `for-pms-reusing.md` and the artifact copy carried the claim with no tracked pointer at all, and several of the 11 pointers it did list had themselves drifted off their lines. Scoping the sweep from the register would have left stale numbers in at least those four files. Register coverage was expanded in the same pass. The `metrics.md` **Baseline (2026-07-06)** line—"40 lifetime evals"—is deliberately unchanged, being a frozen statement about a past measurement. The **Source.** line above it *was* updated, because it describes the current population rather than the baseline: it now reads 36 dedicated files spanning Evals 11 through 46 plus 10 archived files for Evals 1 through 10, correcting an older description under which Evals 3–10 lived only in corrections-log shards.
- **Voice-rules decomposition, at its live sites only.** The index line count 66 → 69 in all four files that publish it (`metrics.md`, `methodology.md`, `retrospection.md`, Case Study #2); the file count 23 → 24 (22 per-rule + 2 supporting) in the two that publish it (`metrics.md`, Case Study #2). **Case Study #3's copies are deliberately left at 23 files / 66 lines / 21 sparse rule numbers.** They sit under the dated heading "Data point 2—Voice-rules per-rule decomposition, 2026-05-07" and describe what that refactor produced; updating them would assert the May decomposition produced `rule-29`, which was in fact authored on 2026-08-06 and carries no slice provenance. The same distinction is now explicit in Case Study #2, which reads "24 reference files (22 per-rule, 2 supporting), 23 of them sliced"—total and sliced are legitimately different numbers, and the previous text conflated them.
- **`qa-checklist.md` 1,016 → 1,096 lines and 51 → 53 H3 subsections.**
- **Case Study #4's memory-budget description corrected in four places, one of which is not a number.** The published text cited `CLAUDE.md ≤3,400` and a single `MEMORY.md ≤3,500`; the real budgets are `CLAUDE.md ≤3,500`, `MEMORY.md` shared core `≤1,000`, and the `MEMORY.local.md` overlay `≤2,500`. The project `CONTEXT.md` budget moved `≤1,800 → ≤2,400`, and the *mechanism* description was stale too: the check has been token-based since mid-2026 and the published "~80-line proxy" no longer describes what runs.

- **The eval gloss corrected, not just the count.** The headline read "46 evals (regression-test records, one per applied job)"; one of the 46 is a skill-improvement session that shipped no application, so the count was right and the definition bound to it was not. `README.md`, `manifesto.md` and the `metrics.md` methodology now say so.
- **"across six release projects" dropped from Case Study #1.** The qualifier was defensible at 40 evals and is not at 46: the six `job-materials` release projects account for a small minority of the eval population, most of which came from the job-search projects. Removed rather than restated, because the source cannot attribute every eval to a project.
- **The detector count corrected to 41, and the counting method with it.** `preflight_check.py` *defines* 42 `detect_` functions but *registers* 41 across its four tier lists; the 42nd appears once in the file—its own `def` line—and is never registered and never runs. The skill settled this on 2026-08-14 and now derives the number from its registry at report time; `SKILL.md` and its ROADMAP both say 41, and the May 2026 ceiling lift was 40 → 41, which the count sits exactly at. So the published 42 counted a function that does not execute. Every live site now reads 41, and `metrics.md` documents the registry method rather than the `def`-line count that produced the error. Two further values surfaced in the same sweep: `for-pms-reusing.md` published "The 39-detector population" three times in one bullet, and Case Study #2 published 40 at line 120 against its own 42 at line 17.
- **Case Study #2's detector paragraph rewritten.** Correcting its 40 was not a number swap—the surrounding argument reasoned from a "working ceiling of 40" that was lifted in May 2026, and the sentences after it narrated the count's approach to that ceiling. The paragraph now carries the history the [metrics dashboard](./metrics.md) already recorded: the count ran against a ceiling of 40 through the v4–v7 rounds, the ceiling was lifted to 41 rather than treated as permanent, and the count sits there. The dated waypoints were re-derived from the detector registry walked backwards through its dated additions, then cross-checked against the project records the registry confirms rather than lifted from the logs wholesale: `metrics.md` and the v5 log both recorded a step that the registry and `job-materials-v7/CONTEXT.md` contradict, because those logs were written under the `def`-line basis the skill later rejected. `metrics.md`'s copy of the same history was corrected alongside it.
- **The metrics baseline date corrected in five places.** `methodology.md` and Case Studies #2, #3 and #4 each ended a paragraph "Baseline date 2026-05-08."; `README.md`'s contents list said "baseline 2026-05-08" on the same line as the link to a dashboard that states **2026-07-06** in six of its own baseline statements. The v2.0.0 re-baseline reached `metrics.md` and none of the five pages citing it.
- **The artifact-tier enumeration completed, in all three places it appears.** `README.md` lists the redacted artifacts twice—once in prose and once as a bulleted index—and `methodology.md` a third time; all three omitted `orchestrate-skill.md` and `sample-enforcement-hook.sh`, and both README copies described a single "hook script" where an observe hook and an enforcement hook ship.

### Changed (2026-08-21)

- **Byte counts retired from published size claims; line counts carry them instead.** Four of the eleven drifted rows were bytes moving while the line count held perfectly steady—the corrections-log index, its two shards, and their total—so on those, bytes produced all of the churn and none of the signal. On the two remaining byte-bearing rows both figures had moved, so retiring bytes costs no signal there either. Bytes are in any case the least informative figure in the portfolio to a reader. `metrics.md`, Case Study #2 and Case Study #3 now publish lines (and H3 counts for `qa-checklist.md`). **Frozen historical byte figures are deliberately kept**—the 194,363-byte corrections-log and 168,338-byte voice-rules monoliths, and the split-moment shard figures—because a frozen value cannot drift and it is half of a before/after comparison that would lose its meaning without it.

### Known and not fixed in this release (2026-08-21)

Named here rather than shipped silently, in three different shapes. The first is a count that cannot be corrected without rewriting the narrative built on top of it, which this project's plan routes to a dedicated session under the operator's disposition rather than the executing session's. The second is a redaction re-port rather than a number edit, and is sequenced separately for that reason. The third is a figure that no longer reproduces and whose cause has not been established—disclosed rather than quietly restated.

- **The hook count is published as ten; there are twelve.** Case Study #4 carries it as prose, as the section heading "The ten hooks", and as one subsection per hook, so a faithful fix writes two new narrative subsections. `metrics.md` carries the same claim as a named inventory with an observe/enforce split that also needs re-deriving; `methodology.md` publishes it a third time ("Ten hook scripts live under `hooks/`"); and `artifacts/policies/publish-portfolio.md` a fourth ("one of the ten hooks registered in…"). All four move together or none does.
- **The artifact tier still publishes the memory budgets this release corrected.** `artifacts/policies/memory-architecture.md` and `artifacts/policies/new-project-intake.md` carry `≤3,400` and `≤1,800` plus the retired "~80-line proxy" framing—the exact figures Case Study #4 was corrected away from—`artifacts/CLAUDE.md` is a stale port showing 14 behavioral primitives and 10 anti-patterns against a live 16 and 14; and `artifacts/sample-skill/SKILL.md` line 354 states 37 mechanical detectors against a live source that says 41—a stale figure inside the body of a re-ported file, and the only detector value this release does not correct. These are redaction re-ports of source policy files rather than number edits, so they are sequenced as their own unit rather than hand-patched here. A reader following the README to the artifact tier will find figures that disagree with the narrative page that sent them until that runs.
- **The corrections-log WRONG-token baseline no longer reproduces.** `metrics.md` publishes a 2026-07-06 baseline of 107 cumulative WRONG tokens across the four corrections-log files; re-derived today with the same method it returns 100. A monotonically-growing proxy should not fall, so either the baseline was mis-taken or the layer lost WRONG-lines to in-place editing—plausible, since across this release's own measurements the line counts held while the bytes grew. The figure is left as published rather than quietly restated, because the cause has not been established and a July snapshot no longer exists to settle it.
