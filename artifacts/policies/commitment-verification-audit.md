...
1. **Enumerate every commitment made during the session.** Start by reading `~/aios/outputs/commitment-logs/{session-id}.jsonl` — populated by `hooks/post-tool-use-verify-write.sh`, this is the ground-truth log of every Write/Edit fired during the session (file_path + tool + timestamp + session-id per line). Enumerate file-write commitments from the log first. Then walk back through the conversation to add commitments NOT captured by the log: declarative-only commitments ("I'll surface this to Craig"), cross-file propagation promises that may not have landed yet, deliverable-presence checks beyond the raw write, and process-rule additions implied by completed task entries. Include both commitments made in chat and commitments implied by completed task entries.
