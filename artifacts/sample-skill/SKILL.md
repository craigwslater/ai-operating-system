...
> **Why it's included.** The composer/verifier pattern is the single most-cited evidence in the portfolio's case-study tier (CS#2 in particular). A reader wanting to verify the "23 evals, ~39 detectors, hundreds of errors caught per month" quantitative spine reads this file alongside CS#2 to see the actual machinery: every workflow step, every detector reference, every encode-into-source rule. This is what "composer + verifier in separate contexts" looks like at production scale, redacted enough to publish without exposing target-company specifics.

*Where to read first.* **Peer PMs**—Step 0 (the eager-load reference set) and Step 5 (the verifier-subagent invocation), plus the rule-promotion lifecycle refs woven through `references/voice-rules/` and `references/qa-checklist.md`. **Recruiters**—[Case Study #2](../../case-studies/02-composer-verifier.md) is the lighter read on the same content; this file shows what the case study describes at production scale.
