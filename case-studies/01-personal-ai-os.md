...
`CLAUDE.md` and the MEMORY files at the root of `~/aios/` carry this layer.

[`artifacts/CLAUDE.md`](../artifacts/CLAUDE.md) is the global behavioral-rules file. Every Claude session reads it first. It defines the priority hierarchy (truth-telling beats data-accuracy beats reasoning-quality beats verify-before-claim-complete beats surgical-discipline beats proactive-issue-surfacing beats persistence-and-auditability beats communication-and-visualization beats efficiency, in that order, with earlier wins on conflict), a list of named behavioral primitives, and a list of anti-patterns each of which Claude is required to recognize and avoid.
...
Versioned releases live at the projects layer. The job-materials skill has shipped through six release projects—`projects/job-materials-v2/`, `v3/`, `v4/`, `v5/`, `v6/`, `v7/`—each closed cleanly with its own retrospective and improvement bundle. The claude-local-frontier meta-project has run to a v3, alongside a dedicated `claude-local-frontier-hooks` project that closed the enforcement half of the hooks architecture; v2 and v3 each ran three planned sessions. Versioning happens because closing a release is the discipline that forces a clean stopping point and an explicit commitment to what the next version owes.

### Layer 3—Policies and templates

`~/aios/policies/` holds sixteen documented workflows. Ten port to the redacted artifact tier: [`session-start.md`](../artifacts/policies/session-start.md), [`session-end.md`](../artifacts/policies/session-end.md), [`new-project-intake.md`](../artifacts/policies/new-project-intake.md), [`commitment-verification-audit.md`](../artifacts/policies/commitment-verification-audit.md), [`file-delivery.md`](../artifacts/policies/file-delivery.md), [`skill-roadmap.md`](../artifacts/policies/skill-roadmap.md), [`publish-portfolio.md`](../artifacts/policies/publish-portfolio.md), [`memory-architecture.md`](../artifacts/policies/memory-architecture.md), [`escalation.md`](../artifacts/policies/escalation.md), and [`autonomy-eligibility.md`](../artifacts/policies/autonomy-eligibility.md). The remaining six—`closing-verifier.md`, `memory-detector-parameters.md`, `post-close-addendum.md`, `pre-write-verification.md`, `sync-private-mirror.md`, and `visual-communication.md`—stay source-side. Not all are step-sequenced: several are reference specs read from inside another workflow rather than run on their own. Each is either exposed as a slash command—`/start-session`, `/end-session`, `/start-project`, `/audit`, `/promote-canonical`, among twenty-five now installed—or invoked from inside one, so the workflow runs the same way every time.

`~/aios/templates/` holds the scaffolding for new artifacts. New project? `project-CLAUDE-md.template` plus `project-CONTEXT-md.template` define the file shape. New skill? `skill-ROADMAP.md.template` defines the canonical structure for tracking open considerations, completed work, and candidate bundles. New eval? `eval-template.md` defines the structure of an applied-and-archived regression record. New product initiative? `product-initiative-CLAUDE-md.template` plus `product-initiative-CONTEXT-md.template` carry the lifecycle-stage field the session-start protocol reads to resolve which stage runs next. Two smaller scaffolds—`correction-entry.template` and `session-log-entry.template`—shape the recurring entries inside those files. Eight templates in all.
...
Dual-environment portability is one axis; running the same operating system across more than one *machine* is the other. The system now spans two Macs—a personal machine and a separate work machine on a company-managed Claude account—from one shared source, without ever letting the two machines' private data touch. The instinct is platform multi-tenancy applied to a personal workflow: **one shared engine, isolated per-machine data planes.**
...
**Versioned project releases.** The job-materials skill has shipped 46 evals. The claude-local-frontier meta-project shipped seven structural improvements across its first two versions; v3 was research-only, producing the ranked improvement roadmap later projects execute. Each release closes with a retrospective and a commitment-verification audit. The discipline produces visible cadence.

**Quantitative output.** The job-materials skill alone runs about 41 detectors against every draft, has shipped 46 evals, and catches and fixes hundreds of errors autonomously per month—measured directly from the corrections logs. Those numbers anchor the headline metric on the [metrics dashboard](../metrics.md). The depth on the operational mechanics—composer/verifier separation, eval-driven correction, hooks as enforcement—lives in [Case Study #2](./02-composer-verifier.md), [Case Study #3](./03-eval-driven-loops.md), and [Case Study #4](./04-discipline-to-machinery.md).
...
The next four case studies go deeper. [Composer/Verifier](./02-composer-verifier.md) walks the two-agent pattern that turned skill drafts into auditable production output. [Eval-Driven Correction Loops](./03-eval-driven-loops.md) walks the WRONG/RIGHT corrections-log discipline and the rule-promotion lifecycle. [From Discipline to Machinery](./04-discipline-to-machinery.md) walks the hooks layer that turned behavioral primitives into runtime checks. [Earned Autonomy](./05-autonomous-execution.md) walks the autonomous-execution engine that runs bounded, low-blast-radius backlogs unattended behind a taxonomy of safety gates.
...
The [metrics dashboard](../metrics.md) is where the system's compounding effect is tracked over time. Current snapshot: 2026-07-06.
...
**Sources:** `~/aios/CLAUDE.md`; `~/aios/MEMORY.md` + `~/aios/MEMORY.local.md` (the shared core and the per-machine overlay); `~/aios/projects/`, `~/aios/policies/`, `~/aios/templates/`, `~/aios/skills/` (architecture pattern); `~/aios/projects/ai-operating-system/inputs/interview-material.md` (origin-moment dialogue verbatim).
