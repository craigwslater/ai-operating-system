...
> **What this is.** The operational dashboard for the personal AI operating system. Four metrics committed at Session 0 of the portfolio project, baselined on 2026-05-19, updated through the signal-triggered sync described in [`README.md`](./README.md).
...
**Baseline (2026-05-19).** Cumulative WRONG token count across the four files: 97 (a rough lower-bound proxy; not every correction uses the WRONG keyword). Headline-rate framing: hundreds per month during active periods.
...
**Source.** `skills/job-materials/evals/` (16 dedicated eval files spanning Evals 11 through 26 at baseline) plus the corrections-log shards (covering Evals 3 through 10, with Evals 1 and 2 archived). Lifetime count: 26 evals.

**Baseline (2026-05-19).** 26 lifetime evals across the job-materials skill. The trailing-90-day cadence will be established at the first dashboard update.
...
**Baseline (2026-05-19).** 40 detectors. Regression-test pass rate: to be established. Pass rate matters because detector count without pass rate can grow misleadingly—adding detectors that always pass because their failure mode is rare is detector inflation.
...
- `skills/job-materials/references/qa-checklist.md`: 989 lines / 129,702 bytes; 50 H3 subsections covering individual checks and gates.
...
**Baseline (2026-05-19).** Cumulative populations as listed above. The trailing-30-day rate will be established at the first dashboard update.
...
**Baseline date: 2026-05-19.** All counts above are computed from the state of `~/.claude-local/` on this date. The next update against this baseline will report deltas plus a 30-day rolling rate where applicable.
