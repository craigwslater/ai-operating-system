...
> **What this is.** The operational dashboard for the personal AI operating system. Four metrics committed at Session 0 of the portfolio project, baselined on 2026-05-08, updated through the signal-triggered sync described in [`README.md`](./README.md).
...
**Headline (Craig's framing, project interview Q6, 2026-05-08): hundreds of errors caught per month easily.**
...
- `references/corrections-log/live-applications.md`—shard for live-application corrections, 572 lines / 72,747 bytes.
- `references/corrections-log-archive.md`—archive for Pre-Eval Drift plus Evals 3 through 5, 192 lines / 14,903 bytes.

Total content under the corrections-log layer at baseline: 1,784 lines / 215,228 bytes. The "hundreds per month" rate refers to the pace at which new corrections enter this layer in months with active job-application work, not the cumulative archive size.
...
**Baseline (2026-05-08).** Cumulative WRONG token count across the four files: 97 (a rough lower-bound proxy; not every correction uses the WRONG keyword). Headline-rate framing per Craig: hundreds per month easily.
...
**Source.** `skills/job-materials/evals/` (13 dedicated eval files spanning Evals 11 through 23 at baseline) plus the corrections-log shards (covering Evals 3 through 10, with Evals 1 and 2 archived). Lifetime count: 23 evals.

**Baseline (2026-05-08).** 23 lifetime evals across the job-materials skill. The trailing-90-day cadence will be established at the first dashboard update.
...
**Source.** `skills/job-materials/scripts/preflight_check.py` (39 `detect_*` functions at baseline, verified by counting function definitions). The cross-cutting reference is the v5 Session 3 close on 2026-05-06, which closed at 39 detectors within a working ceiling of 40.

**Baseline (2026-05-08).** 39 detectors. Regression-test pass rate: to be established. Pass rate matters because detector count without pass rate can grow misleadingly—adding detectors that always pass because their failure mode is rare is detector inflation.
...
- `skills/job-materials/references/qa-checklist.md`: 849 lines / 106,708 bytes; 43 H3 subsections covering individual checks and gates.
...
**Baseline (2026-05-08).** Cumulative populations as listed above. The trailing-30-day rate will be established at the first dashboard update.
...
**Baseline date: 2026-05-08.** All counts above are computed from the state of `~/.claude-local/` on this date. The next update against this baseline will report deltas plus a 30-day rolling rate where applicable.

**Source verification.** Each numeric claim above traces to a specific source file or directory. The redacted artifact tier under [`artifacts/`](./artifacts/) provides a reader with the operational files that produced the numbers, with PII, target-company names, third-party individuals, and prior-employer narrative redacted but the structural depth preserved. Any number on this page can be verified against the artifacts before being taken at face value.
